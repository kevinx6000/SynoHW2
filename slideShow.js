...
// Create navbar
Slide.prototype.createNavbar = function() {
	var i = 0;
	var output = "";
...
// Change image
Slide.prototype.changeImage = function(shift) {
	var i = 0;
	var curImgB = {};
...
// Adjust image size
Slide.prototype.adjustImageSize = function(imgID) {
	var imgBlk = {};
	var imgSrc = {};
...
// Adjust navbar size
Slide.prototype.adjustNavbarSize = function() {
	var i = 0;
	var navImg = {};
	var navBox = {};
...
// Adjust navbar position
Slide.prototype.adjustNavbarPosition = function(isDirect) {
	var i = 0;
	var navbar = {};
	var navbox = {};
	...
	this.navOldPos = this.navCurPos.slice();
	...
	// Apply to navboxes
	shift = this.navCurPos[this.curImgID] - this.navOldPos[this.curImgID];
	for (i = 0; i < this.items.length; i++) {
		navbox = document.getElementById("navbox" + i);
		navbox.style.opacity = ((i == this.curImgID) ? "1" : "0.3");

		// First time or specified, update positions directly
		if (this.navOldPos[i] == undefined || isDirect) {
			navbox.style.left = this.navCurPos[i] + "px";
		} else {

			// Animation for middle boxes
			if ((this.navCurPos[this.curImgID] - this.navOldPos[this.curImgID]) * 
					(this.navCurPos[i] - this.navOldPos[i]) >= 0) {
				this.shiftAnimation(navbox, this.navOldPos[i], this.navCurPos[i], 300);
				...
				this.shiftAnimation(navbox, this.navOldPos[i], this.navOldPos[i] + shift, 150, function(obj, id) {
					return function() {
						that.shiftAnimation(obj, that.navCurPos[id] - shift, that.navCurPos[id], 150);
					}
				}(navbox, i));
			}
		}
...
Slide.prototype.adjustToParent = function(chd, par) {
...
Slide.prototype.shiftAnimation = function(obj, oldPos, newPos, msec, func) {
